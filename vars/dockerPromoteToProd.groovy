                    ...
                    dir("${env.PROMOTION_WORKDIR}") {
                        script {
                            env.DEV_IMAGE_REPOSITORY = sh(
                                returnStdout: true,
                                script: 'yq -r \'.image.repository // ""\' "$VALUES_DEV_PATH"'
                            ).trim()
                            env.PROMOTED_IMAGE_TAG = sh(
                                returnStdout: true,
                                script: 'yq -r \'.image.tag // ""\' "$VALUES_DEV_PATH"'
                            ).trim()

                            if (!env.DEV_IMAGE_REPOSITORY || env.DEV_IMAGE_REPOSITORY == 'null') {
                                error("Missing .image.repository in ${params.VALUES_DEV_PATH}")
                            }
                            if (!env.PROMOTED_IMAGE_TAG || env.PROMOTED_IMAGE_TAG == 'null') {
                                error("Missing .image.tag in ${params.VALUES_DEV_PATH}")
                            }
                    ...
                    dir("${env.PROMOTION_WORKDIR}") {
                        withCredentials([
                            ...
                            sh '''
                                ...
                                export IMAGE_REPOSITORY="$PROD_IMAGE_REPOSITORY"
                                export IMAGE_TAG="$PROMOTED_IMAGE_TAG"
                                yq -i '.image.repository = strenv(IMAGE_REPOSITORY) | .image.tag = strenv(IMAGE_TAG)' "$VALUES_PROD_PATH"
